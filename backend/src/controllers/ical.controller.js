  ...
  try {
    ...
    if (!Number.isFinite(id)) {
      return res
        ...
        .json(errorResponse("Invalid property id", HTTP_STATUS.BAD_REQUEST));
    }
    const list = Array.isArray(urls) && urls.length ? urls : url ? [url] : [];
    if (
      list.length === 0 ||
      list.some((u) => !/^https?:\/\//i.test(String(u)))
    ) {
      return res
        .status(HTTP_STATUS.BAD_REQUEST)
        .json(errorResponse("Invalid iCal URL(s)", HTTP_STATUS.BAD_REQUEST));
    }
    ...
    return res
      ...
      .json(
        errorResponse("Failed to sync iCal", HTTP_STATUS.INTERNAL_SERVER_ERROR)
      );
  }
};
...
export const getPropertyIcalBlocks = async (req, res) => {
  try {
    ...
    const cached = IcalCache.get(id);
    if (!cached)
      return res.json({ success: true, message: "No cached iCal", data: [] });
    if (!from || !to)
      return res.json({
        success: true,
        message: "All cached events",
        data: cached.events,
      });
    const s = new Date(String(from));
    const e = new Date(String(to));
    const list = (cached.events || []).filter(
  ...
  try {
    ...
    if (!property) {
      return res
        ...
        .json(errorResponse("Property not found", HTTP_STATUS.NOT_FOUND));
    }
    ...
    createEvents(events, (err, ics) => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="property-${property.id}-availability.ics"`
      );
      return res.send(ics);
    });
  } catch (error) {
...
export const exportBookingICS = async (req, res) => {
  try {
    ...
    if (!Number.isFinite(id)) {
      return res
        .status(HTTP_STATUS.BAD_REQUEST)
        .json(errorResponse("Invalid booking id", HTTP_STATUS.BAD_REQUEST));
    }
    ...
    if (!booking) {
      return res
        .status(HTTP_STATUS.NOT_FOUND)
        .json(errorResponse("Booking not found", HTTP_STATUS.NOT_FOUND));
    }
    ...
    createEvents([event], (err, ics) => {
      if (err) {
        ...
        return res
          ...
          .json(
            errorResponse(
              "Failed to generate booking event",
              HTTP_STATUS.INTERNAL_SERVER_ERROR
            )
          );
      }
